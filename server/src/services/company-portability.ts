...
import { ensureOpenCodeModelConfiguredAndAvailable } from "@partyclipai/adapter-opencode-local/server";
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".partyclip.yaml" || normalized === ".partyclip.yml") return "extension";
...
const ADAPTER_DEFAULT_RULES_BY_TYPE: Record<string, Array<{ path: string[]; value: unknown }>> = {
  ...
  openclaw_gateway: [
    ...
    { path: ["sessionKey"], value: "partyclip" },
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .partyclip.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .partyclip.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .partyclip.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .partyclip.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .partyclip.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .partyclip.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .partyclip.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .partyclip.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .partyclip.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .partyclip.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .partyclip.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .partyclip.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .partyclip.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[partyclipExtensionPath];
  if (selectedFiles.has(partyclipExtensionPath) && typeof extensionEntry === "string") {
    filtered[partyclipExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }
...
function findPaperclipExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".partyclip.yaml"] === "string") return ".partyclip.yaml";
  if (typeof files[".partyclip.yml"] === "string") return ".partyclip.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.partyclip.yaml") || entry.endsWith("/.partyclip.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "partyclip_bundled") {
    ...
    return {
      ...
      commit,
      trackingRef: "master",
      url: `https://github.com/partyclipai/partyclip/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "partyclip_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.partyclipSkillKey = skill.key;
  metadata.partyclip = {
    ...(isPlainRecord(metadata.partyclip) ? metadata.partyclip : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const partyclipExtensionPath = findPaperclipExtensionPath(normalizedFiles);
  const partyclipExtension = partyclipExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, partyclipExtensionPath) ?? "")
    : {};
  const partyclipCompany = isPlainRecord(partyclipExtension.company) ? partyclipExtension.company : {};
  const partyclipSidebar = normalizePortableSidebarOrder(partyclipExtension.sidebar);
  const partyclipAgents = isPlainRecord(partyclipExtension.agents) ? partyclipExtension.agents : {};
  const partyclipProjects = isPlainRecord(partyclipExtension.projects) ? partyclipExtension.projects : {};
  const partyclipTasks = isPlainRecord(partyclipExtension.tasks) ? partyclipExtension.tasks : {};
  const partyclipRoutines = isPlainRecord(partyclipExtension.routines) ? partyclipExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(partyclipCompany.brandColor),
      logoPath: asString(partyclipCompany.logoPath) ?? asString(partyclipCompany.logo),
      attachmentMaxBytes:
        typeof partyclipCompany.attachmentMaxBytes === "number" && Number.isFinite(partyclipCompany.attachmentMaxBytes)
          ? Math.max(1, Math.floor(partyclipCompany.attachmentMaxBytes))
          : null,
      requireBoardApprovalForNewAgents:
        typeof partyclipCompany.requireBoardApprovalForNewAgents === "boolean"
          ? partyclipCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
      feedbackDataSharingEnabled:
        typeof partyclipCompany.feedbackDataSharingEnabled === "boolean"
          ? partyclipCompany.feedbackDataSharingEnabled
          : false,
      feedbackDataSharingConsentAt:
        typeof partyclipCompany.feedbackDataSharingConsentAt === "string"
          ? partyclipCompany.feedbackDataSharingConsentAt
          : null,
      feedbackDataSharingConsentByUserId:
        asString(partyclipCompany.feedbackDataSharingConsentByUserId),
      feedbackDataSharingTermsVersion:
        asString(partyclipCompany.feedbackDataSharingTermsVersion),
    },
    sidebar: partyclipSidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(partyclipAgents[slug]) ? partyclipAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(partyclipProjects[slug]) ? partyclipProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(partyclipTasks[slug]) ? partyclipTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(partyclipRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(partyclipRoutines[slug]) ? partyclipRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
    ...
    const candidatePaths = (tree.tree ?? [])
      ...
      .filter((entry) => {
        ...
        return (
          ...
          relative === ".partyclip.yaml" ||
          relative === ".partyclip.yml"
        );
      });
  ...
  async function exportBundle(
    ...
    const partyclipAgentsOut: Record<string, Record<string, unknown>> = {};
    const partyclipProjectsOut: Record<string, Record<string, unknown>> = {};
    const partyclipTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const partyclipRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        partyclipAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      partyclipProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      partyclipTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      partyclipRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const partyclipExtensionPath = ".partyclip.yaml";
    const partyclipAgents = Object.fromEntries(
      Object.entries(partyclipAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const partyclipProjects = Object.fromEntries(
      Object.entries(partyclipProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const partyclipTasks = Object.fromEntries(
      Object.entries(partyclipTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const partyclipRoutines = Object.fromEntries(
      Object.entries(partyclipRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[partyclipExtensionPath] = buildYamlFile(
      {
        schema: "partyclip/v1",
        ...
        agents: Object.keys(partyclipAgents).length > 0 ? partyclipAgents : undefined,
        projects: Object.keys(partyclipProjects).length > 0 ? partyclipProjects : undefined,
        tasks: Object.keys(partyclipTasks).length > 0 ? partyclipTasks : undefined,
        routines: Object.keys(partyclipRoutines).length > 0 ? partyclipRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, partyclipExtensionPath);
