...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __partyclip_exports: module.exports,
      __partyclip_module: module,
      __partyclip_require: requireInSandbox,
      __partyclip_filename: realPath,
      __partyclip_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__partyclip_exports, __partyclip_module, __partyclip_require, __partyclip_filename, __partyclip_dirname)`;
