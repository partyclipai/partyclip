 ...
 * ~/.partyclip/adapter-plugins.json. This is the source of truth for which
...
export interface AdapterPluginRecord {
  /** npm package name (e.g., "droid-partyclip-adapter") */
  packageName: string;
...
function adapterPluginPaths() {
  const partyclipDir = resolvePaperclipHomeDir();
  return {
    adapterPluginsDir: path.join(partyclipDir, "adapter-plugins"),
    adapterPluginsStorePath: path.join(partyclipDir, "adapter-plugins.json"),
    adapterSettingsPath: path.join(partyclipDir, "adapter-settings.json"),
  };
}
...
function ensureDirs(): string {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    fs.writeFileSync(pkgJsonPath, JSON.stringify({
      name: "partyclip-adapter-plugins",
      version: "0.0.0",
      private: true,
