 ...
 *    (`~/.partyclip/plugins/`) and `node_modules` for packages matching
 *    the `partyclip-plugin-*` naming convention. Aggregates results with
 ...
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "partyclip-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: PartyclipPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.partyclip/plugins/ local directory
  | "npm"               // npm packages matching partyclip-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: PartyclipPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the partyclip-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "partyclip-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: PartyclipPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
   ...
   * Node.js environment matching the "partyclip-plugin-*" naming convention.
   *
   ...
   * the "partyclipPlugin.manifest" key, loads the manifest module, and
   ...
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<PartyclipPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: PartyclipPluginManifestV1;
    newManifest: PartyclipPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
 ...
 * Accepts both the "partyclip-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @partyclipai/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "partyclipPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const partyclipPlugin = pkgJson["partyclipPlugin"];
  if (
    partyclipPlugin !== null &&
    typeof partyclipPlugin === "object" &&
    !Array.isArray(partyclipPlugin)
  ) {
    const manifestRelPath = (partyclipPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: PartyclipPluginManifestV1): string | undefined {
 ...
 *   packageName: "partyclip-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: PartyclipPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as PartyclipPluginManifestV1 | null;
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<PartyclipPluginManifestV1> {
    let raw: unknown;
  ...
  async function buildDiscoveredPlugin(
    ...
    const hasPaperclipPlugin = "partyclipPlugin" in pkgJson;
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<PartyclipPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: PartyclipPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @partyclipai/shared exports). Run those workers through
