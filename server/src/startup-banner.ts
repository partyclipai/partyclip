...
import type { BindMode, DeploymentExposure, DeploymentMode } from "@partyclipai/shared";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.PARTYCLIP_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.PARTYCLIP_AGENT_JWT_SECRET === "string" ? parsed.PARTYCLIP_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm partyclipai onboard`)",
  };
}
