...
import { execute, testEnvironment } from "@partyclipai/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@partyclipai/adapter-openclaw-gateway/ui";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
        ...
        buildContext(
          ...
          },
          {
            ...
            context: {
              ...
              partyclipRuntimeServiceIntents: [
                {
                  name: "preview",
                  lifecycle: "ephemeral",
                },
      ...
      expect(payload?.sessionKey).toBe("partyclip:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("PARTYCLIP_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("PARTYCLIP_TASK_ID=task-123");
      ...
      expect(payload?.partyclip).toMatchObject({
        wake: {
          latestCommentId: "comment-2",
          commentIds: ["comment-1", "comment-2"],
        },
