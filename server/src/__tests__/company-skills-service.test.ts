...
import { companies, companySkills, createDb } from "@partyclipai/db";
import {
...
describeEmbeddedPostgres("companySkillService.list", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("partyclip-company-skills-service-");
  ...
  it("lists skills without exposing markdown content", async () => {
    ...
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "partyclip-heavy-skill-"));
