...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "partyclip", LOGNAME: "partyclip", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "partyclip" }),
      },
    );

    expect(tokens).toEqual(["partyclip", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "partyclip" }, {
        userInfo: () => ({ username: "partyclip" }),
      });

      expect(tokens).toEqual(["partyclip", "custom-token"]);
    } finally {
