...
describe("agent local JWT", () => {
  const secretEnv = "PARTYCLIP_AGENT_JWT_SECRET";
  const betterAuthSecretEnv = "BETTER_AUTH_SECRET";
  const ttlEnv = "PARTYCLIP_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "PARTYCLIP_AGENT_JWT_ISSUER";
  const audienceEnv = "PARTYCLIP_AGENT_JWT_AUDIENCE";
  ...
  it("falls back to BETTER_AUTH_SECRET when PARTYCLIP_AGENT_JWT_SECRET is absent", () => {
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "partyclip";
    process.env[audienceEnv] = "partyclip-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
