...
const ORIGINAL_PARTYCLIP_RUNTIME_API_CANDIDATES_JSON = process.env.PARTYCLIP_RUNTIME_API_CANDIDATES_JSON;
...
function buildTestConfig(overrides: Record<string, unknown> = {}) {
  return {
    ...
    databaseUrl: "postgres://partyclip:partyclip@127.0.0.1:5432/partyclip",
    embeddedPostgresDataDir: "/tmp/partyclip-test-db",
    embeddedPostgresPort: 54329,
    databaseBackupEnabled: false,
    databaseBackupIntervalMinutes: 60,
    databaseBackupRetentionDays: 30,
    databaseBackupDir: "/tmp/partyclip-test-backups",
    serveUi: false,
...
describe("startServer PARTYCLIP_API_URL handling", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    loadConfigMock.mockReturnValue(buildTestConfig());
    process.env.BETTER_AUTH_SECRET = "test-secret";
  ...
  afterEach(() => {
    ...
    if (ORIGINAL_PARTYCLIP_RUNTIME_API_CANDIDATES_JSON === undefined) {
      delete process.env.PARTYCLIP_RUNTIME_API_CANDIDATES_JSON;
    } else {
      process.env.PARTYCLIP_RUNTIME_API_CANDIDATES_JSON = ORIGINAL_PARTYCLIP_RUNTIME_API_CANDIDATES_JSON;
    }
  ...
  it("uses the externally set PARTYCLIP_API_URL when provided", async () => {
    process.env.PARTYCLIP_API_URL = "http://custom-api:3100";

    const started = await startServer();

    expect(started.apiUrl).toBe("http://custom-api:3100");
    expect(process.env.PARTYCLIP_API_URL).toBe("http://custom-api:3100");
    expect(JSON.parse(process.env.PARTYCLIP_RUNTIME_API_CANDIDATES_JSON ?? "[]")).toEqual(
      expect.arrayContaining(["http://custom-api:3100"]),
    );
    expect(JSON.parse(process.env.PARTYCLIP_RUNTIME_API_CANDIDATES_JSON ?? "[]")[0]).toBe("http://custom-api:3100");
  });

  it("falls back to host-based URL when PARTYCLIP_API_URL is not set", async () => {
    const started = await startServer();

    expect(started.apiUrl).toBe("http://127.0.0.1:3210");
    expect(process.env.PARTYCLIP_API_URL).toBe("http://127.0.0.1:3210");
  });
  ...
  it("rewrites explicit-port auth public URLs when detect-port selects a new port", async () => {
    ...
    expect(process.env.PARTYCLIP_RUNTIME_API_URL).toBe("http://my-host.ts.net:3110");
  });
  ...
  it("keeps no-port auth public URLs stable when detect-port selects a new port", async () => {
    loadConfigMock.mockReturnValueOnce(buildTestConfig({
      port: 3100,
      authBaseUrlMode: "explicit",
      authPublicBaseUrl: "https://partyclip.example",
    }));
    detectPortMock.mockResolvedValueOnce(3110);
    ...
    expect(started.listenPort).toBe(3110);
    expect(started.apiUrl).toBe("https://partyclip.example");
    expect(process.env.PARTYCLIP_RUNTIME_API_URL).toBe("https://partyclip.example");
  });
});
