...
import { createAcpxLocalExecutor } from "@partyclipai/adapter-acpx-local/server";
import type {
  AcpRuntime,
  AcpRuntimeEvent,
  AcpRuntimeHandle,
...
async function createRuntimeSkill(root: string, input: {
  ...
  const runtimeName = input.runtimeName ?? "partyclip-test-skill";
...
function buildContext(root: string, overrides: Partial<AdapterExecutionContext> = {}): AdapterExecutionContext {
  return {
    ...
    context: {
      issueId: "issue-1",
      partyclipTaskMarkdown: "Task context",
    },
    onLog: async () => {},
    ...overrides,
  };
...
describe("acpx_local execute", () => {
  it("streams ACPX session, status, text, and tool events before returning success", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "partyclip-acpx-success-"));
    try {
  ...
  it("reuses a compatible warm session and starts fresh when cwd changes", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "partyclip-acpx-reuse-"));
  ...
  it("closes duplicate warm handles from concurrent runs for the same session key", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "partyclip-acpx-concurrent-"));
    try {
      const runtimes: FakeRuntime[] = [];
      ...
      expect(runtimes.flatMap((runtime) => runtime.closeInputs).filter((input) =>
        input.reason === "partyclip duplicate warm handle cleanup"
      )).toHaveLength(1);
    } finally {
      await fs.rm(root, { recursive: true, force: true });
    }
  ...
  it("retries with a fresh session when ACPX cannot resume the saved backend session", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "partyclip-acpx-resume-"));
    try {
      const runtime = new FakeRuntime({} as AcpRuntimeOptions);
  ...
  it("cancels and closes stale handles on timeout", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "partyclip-acpx-timeout-"));
    try {
  ...
  it("returns structured auth errors", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "partyclip-acpx-error-"));
    try {
      const runtime = new FakeRuntime({} as AcpRuntimeOptions);
  ...
  it("returns structured ACP protocol errors", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "partyclip-acpx-protocol-"));
    try {
      const runtime = new FakeRuntime({} as AcpRuntimeOptions);
  ...
  it("materializes selected skills for ACPX Claude and passes public session metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "partyclip-acpx-claude-skills-"));
    try {
  ...
  it("includes skill content in the ACPX Claude session fingerprint", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "partyclip-acpx-claude-fingerprint-"));
    try {
  ...
  it("materializes selected skills into the effective ACPX Codex CODEX_HOME", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "partyclip-acpx-codex-skills-"));
    try {
      const skill = await createRuntimeSkill(root, {});
  ...
  it("keeps ACPX custom skill selection tracked without runtime materialization", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "partyclip-acpx-custom-skills-"));
    try {
      const skill = await createRuntimeSkill(root, {});
      let runtime: FakeRuntime | null = null;
      let meta: Record<string, unknown> | null = null;
