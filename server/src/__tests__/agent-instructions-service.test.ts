...
describe("agent instructions service", () => {
  const originalPaperclipHome = process.env.PARTYCLIP_HOME;
  const originalPaperclipInstanceId = process.env.PARTYCLIP_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalPaperclipHome === undefined) delete process.env.PARTYCLIP_HOME;
    else process.env.PARTYCLIP_HOME = originalPaperclipHome;
    if (originalPaperclipInstanceId === undefined) delete process.env.PARTYCLIP_INSTANCE_ID;
    else process.env.PARTYCLIP_INSTANCE_ID = originalPaperclipInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const partyclipHome = await makeTempDir("partyclip-agent-instructions-home-");
    const externalRoot = await makeTempDir("partyclip-agent-instructions-external-");
    cleanupDirs.add(partyclipHome);
    cleanupDirs.add(externalRoot);
    process.env.PARTYCLIP_HOME = partyclipHome;
    process.env.PARTYCLIP_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("partyclip-agent-instructions-new-external-");
    cleanupDirs.add(partyclipHome);
    cleanupDirs.add(externalRoot);
    process.env.PARTYCLIP_HOME = partyclipHome;
    process.env.PARTYCLIP_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("partyclip-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const partyclipHome = await makeTempDir("partyclip-agent-instructions-recover-");
    cleanupDirs.add(partyclipHome);
    process.env.PARTYCLIP_HOME = partyclipHome;
    process.env.PARTYCLIP_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const partyclipHome = await makeTempDir("partyclip-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("partyclip-agent-instructions-stale-root-");
    cleanupDirs.add(partyclipHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const partyclipHome = await makeTempDir("partyclip-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("partyclip-agent-instructions-heal-write-stale-");
    cleanupDirs.add(partyclipHome);
    cleanupDirs.add(staleRoot);
    process.env.PARTYCLIP_HOME = partyclipHome;
    process.env.PARTYCLIP_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const partyclipHome = await makeTempDir("partyclip-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("partyclip-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(partyclipHome);
    cleanupDirs.add(staleRoot);
    process.env.PARTYCLIP_HOME = partyclipHome;
    process.env.PARTYCLIP_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const partyclipHome = await makeTempDir("partyclip-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("partyclip-agent-instructions-partial-root-");
    cleanupDirs.add(partyclipHome);
    cleanupDirs.add(staleRoot);
    process.env.PARTYCLIP_HOME = partyclipHome;
    process.env.PARTYCLIP_INSTANCE_ID = "test-instance";
