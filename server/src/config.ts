...
const PARTYCLIP_ENV_FILE_PATH = resolvePaperclipEnvPath();
if (existsSync(PARTYCLIP_ENV_FILE_PATH)) {
  loadDotenv({ path: PARTYCLIP_ENV_FILE_PATH, override: false, quiet: true });
}

const CWD_ENV_PATH = resolve(process.cwd(), ".env");
const isSameFile = existsSync(CWD_ENV_PATH) && existsSync(PARTYCLIP_ENV_FILE_PATH)
  ? realpathSync(CWD_ENV_PATH) === realpathSync(PARTYCLIP_ENV_FILE_PATH)
  : CWD_ENV_PATH === PARTYCLIP_ENV_FILE_PATH;
...
function detectTailnetBindHost(): string | undefined {
  const explicit = process.env.PARTYCLIP_TAILNET_BIND_HOST?.trim();
...
export function loadConfig(): Config {
  ...
  const strictModeFromEnv = process.env.PARTYCLIP_SECRETS_STRICT_MODE;
  const secretsStrictMode =
    strictModeFromEnv !== undefined
      ? strictModeFromEnv === "true"
      : (fileSecrets?.strictMode ?? false);

  const providerFromEnvRaw = process.env.PARTYCLIP_SECRETS_PROVIDER;
  ...
  const storageProviderFromEnvRaw = process.env.PARTYCLIP_STORAGE_PROVIDER;
  ...
  const storageS3ForcePathStyle =
    process.env.PARTYCLIP_STORAGE_S3_FORCE_PATH_STYLE !== undefined
      ? process.env.PARTYCLIP_STORAGE_S3_FORCE_PATH_STYLE === "true"
      : (fileStorage?.s3?.forcePathStyle ?? false);
  const feedbackExportBackendUrl =
    process.env.PARTYCLIP_FEEDBACK_EXPORT_BACKEND_URL?.trim() ||
    process.env.PARTYCLIP_TELEMETRY_BACKEND_URL?.trim() ||
    undefined;
  const feedbackExportBackendToken =
    process.env.PARTYCLIP_FEEDBACK_EXPORT_BACKEND_TOKEN?.trim() ||
    process.env.PARTYCLIP_TELEMETRY_BACKEND_TOKEN?.trim() ||
    undefined;

  const deploymentModeFromEnvRaw = process.env.PARTYCLIP_DEPLOYMENT_MODE;
  const deploymentModeFromEnv =
    deploymentModeFromEnvRaw && DEPLOYMENT_MODES.includes(deploymentModeFromEnvRaw as DeploymentMode)
      ? (deploymentModeFromEnvRaw as DeploymentMode)
      : null;
  const deploymentMode: DeploymentMode = deploymentModeFromEnv ?? fileConfig?.server.deploymentMode ?? "local_trusted";
  const deploymentExposureFromEnvRaw = process.env.PARTYCLIP_DEPLOYMENT_EXPOSURE;
  ...
  const bindFromEnvRaw = process.env.PARTYCLIP_BIND;
  const bindFromEnv =
  ...
  const customBindHost = process.env.PARTYCLIP_BIND_HOST ?? fileConfig?.server.customBindHost;
  const authBaseUrlModeFromEnvRaw = process.env.PARTYCLIP_AUTH_BASE_URL_MODE;
  const authBaseUrlModeFromEnv =
    authBaseUrlModeFromEnvRaw &&
    AUTH_BASE_URL_MODES.includes(authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      ? (authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      : null;
  const publicUrlFromEnv = process.env.PARTYCLIP_PUBLIC_URL;
  ...
  const disableSignUpFromEnv = process.env.PARTYCLIP_AUTH_DISABLE_SIGN_UP;
  const authDisableSignUp: boolean =
    disableSignUpFromEnv !== undefined
      ? disableSignUpFromEnv === "true"
      : (fileConfig?.auth?.disableSignUp ?? false);
  const allowedHostnamesFromEnvRaw = process.env.PARTYCLIP_ALLOWED_HOSTNAMES;
  ...
  const companyDeletionEnvRaw = process.env.PARTYCLIP_ENABLE_COMPANY_DELETION;
  const companyDeletionEnabled =
    companyDeletionEnvRaw !== undefined
      ? companyDeletionEnvRaw === "true"
      : deploymentMode === "local_trusted";
  const databaseBackupEnabled =
    process.env.PARTYCLIP_DB_BACKUP_ENABLED !== undefined
      ? process.env.PARTYCLIP_DB_BACKUP_ENABLED === "true"
      : (fileDatabaseBackup?.enabled ?? true);
  const databaseBackupIntervalMinutes = Math.max(
    1,
    Number(process.env.PARTYCLIP_DB_BACKUP_INTERVAL_MINUTES) ||
      fileDatabaseBackup?.intervalMinutes ||
      60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    Number(process.env.PARTYCLIP_DB_BACKUP_RETENTION_DAYS) ||
      fileDatabaseBackup?.retentionDays ||
      7,
  );
  const databaseBackupDir = resolveHomeAwarePath(
    process.env.PARTYCLIP_DB_BACKUP_DIR ??
  ...
  return {
    ...
    uiDevMiddleware: process.env.PARTYCLIP_UI_DEV_MIDDLEWARE === "true",
    secretsProvider,
    secretsStrictMode,
