...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@partyclipai/db";
import type { DeploymentMode } from "@partyclipai/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  partyclipUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).partyclipUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.partyclipUpgradeContext = context;
