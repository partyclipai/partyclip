...
- Check wake context: `PARTYCLIP_TASK_ID`, `PARTYCLIP_WAKE_REASON`, `PARTYCLIP_WAKE_COMMENT_ID`.
...
If `PARTYCLIP_APPROVAL_ID` is set:
...
- If `PARTYCLIP_TASK_ID` is set and assigned to you, prioritize that task.
...
- Use `partyclip-create-agent` skill when hiring new agents.
...
- Always include `X-Partyclip-Run-Id` header on mutating API calls.
