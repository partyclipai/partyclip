...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.PARTYCLIP_SECRETS_PROVIDER === undefined) {
    process.env.PARTYCLIP_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.PARTYCLIP_SECRETS_STRICT_MODE === undefined) {
    process.env.PARTYCLIP_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.PARTYCLIP_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.PARTYCLIP_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.PARTYCLIP_MIGRATION_AUTO_APPLY === "true") return true;
    if (process.env.PARTYCLIP_MIGRATION_PROMPT === "never") return false;
      ...
      if (!apply) {
        throw new Error(
          `${label} has pending migrations (${formatPendingMigrationSummary(state.pendingMigrations)}). ` +
            "Refusing to start against a stale schema. Run pnpm db:migrate or set PARTYCLIP_MIGRATION_AUTO_APPLY=true.",
        );
      }
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@partyclip.local";
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.PARTYCLIP_EMBEDDED_POSTGRES_VERBOSE === "true";
    ...
    if (runningPid) {
      ...
      const configuredAdminConnectionString = `postgres://partyclip:partyclip@127.0.0.1:${configuredPort}/postgres`;
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "partyclip");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://partyclip:partyclip@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "partyclip");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: partyclip");
    }
  
    const embeddedConnectionString = `postgres://partyclip:partyclip@127.0.0.1:${port}/partyclip`;
  ...
  const configuredApiUrl = process.env.PARTYCLIP_API_URL?.trim() || runtimeApiUrl;
  ...
  process.env.PARTYCLIP_LISTEN_HOST = runtimeListenHost;
  process.env.PARTYCLIP_LISTEN_PORT = String(listenPort);
  process.env.PARTYCLIP_RUNTIME_API_URL = runtimeApiUrl;
  process.env.PARTYCLIP_RUNTIME_API_CANDIDATES_JSON = JSON.stringify(runtimeApiCandidates);
  process.env.PARTYCLIP_API_URL = configuredApiUrl;
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.PARTYCLIP_OPEN_ON_LISTEN === "true") {
