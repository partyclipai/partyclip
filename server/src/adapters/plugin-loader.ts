...
function extractUiParserSource(
  ...
  const contractVersion = pkg.partyclip?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no partyclip.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
