...
const hermesLocalAdapter: ServerAdapterModule = {
  ...
  execute: async (ctx) => {
    ...
    const authGuardPrompt = [
      ...
      "Use X-Partyclip-Run-Id: $PARTYCLIP_RUN_ID on every Paperclip API request that writes or mutates data, including comments and issue updates.",
...
const externalAdaptersReady: Promise<void> = (async () => {
  try {
    ...
    for (const externalAdapter of externalAdapters) {
      ...
      if (overriding) {
        console.log(
          `[partyclip] External adapter "${externalAdapter.type}" overrides built-in adapter`,
        );
    ...
    console.error("[partyclip] Failed to load external adapters:", err);
  }
})();
...
export function setOverridePaused(type: string, paused: boolean): boolean {
  ...
  if (paused && !wasPaused) {
    ...
    console.log(`[partyclip] Override paused for "${type}" — builtin adapter restored`);
    return true;
  }
  if (!paused && wasPaused) {
    pausedOverrides.delete(type);
    console.log(`[partyclip] Override resumed for "${type}" — external adapter active`);
    return true;
  }
  return false;
}
