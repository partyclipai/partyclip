const FAVICON_BLOCK_START = "<!-- PARTYCLIP_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- PARTYCLIP_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- PARTYCLIP_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- PARTYCLIP_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.PARTYCLIP_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.PARTYCLIP_WORKTREE_NAME) ?? nonEmpty(env.PARTYCLIP_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.PARTYCLIP_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="partyclip-worktree-enabled" content="true" />',
    `<meta name="partyclip-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="partyclip-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="partyclip-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
