...
Environment overrides:
  ...
  PARTYCLIP_HOST=127.0.0.1    Host passed to the onboarded server
  PARTYCLIP_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/partyclip-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/partyclip-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export PARTYCLIP_HOST="${PARTYCLIP_HOST:-127.0.0.1}"
export PARTYCLIP_PORT="${PARTYCLIP_PORT:-3100}"
export PARTYCLIP_OPEN_ON_LISTEN="${PARTYCLIP_OPEN_ON_LISTEN:-false}"
...
echo "PARTYCLIP_HOST: $PARTYCLIP_HOST"
echo "PARTYCLIP_PORT: $PARTYCLIP_PORT"
...
env \
  ...
  HOST="$PARTYCLIP_HOST" \
  PORT="$PARTYCLIP_PORT" \
  PARTYCLIP_OPEN_ON_LISTEN="$PARTYCLIP_OPEN_ON_LISTEN" \
  pnpm partyclipai onboard --yes --data-dir "$PC_DATA"
