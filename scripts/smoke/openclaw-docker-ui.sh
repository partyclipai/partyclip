...
PARTYCLIP_HOST_PORT="${PARTYCLIP_HOST_PORT:-3100}"
PARTYCLIP_HOST_FROM_CONTAINER="${PARTYCLIP_HOST_FROM_CONTAINER:-host.docker.internal}"
...
detect_partyclip_base_url() {
  ...
  for candidate in "$PARTYCLIP_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    ...
    health_url="http://${candidate}:${PARTYCLIP_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${PARTYCLIP_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${PARTYCLIP_HOST_PORT}
  (Do not use http://127.0.0.1:${PARTYCLIP_HOST_PORT} inside the container.)
  If Paperclip rejects the host, run on host machine:
    pnpm partyclipai allowed-hostname host.docker.internal
