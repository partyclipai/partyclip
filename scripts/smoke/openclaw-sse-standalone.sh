...
PARTYCLIP_COMPANY_ID="${PARTYCLIP_COMPANY_ID:-openclaw-smoke-company}"
PARTYCLIP_API_URL="${PARTYCLIP_API_URL:-http://localhost:3100}"
PARTYCLIP_TASK_ID="${PARTYCLIP_TASK_ID:-openclaw-smoke-task}"
PARTYCLIP_WAKE_REASON="${PARTYCLIP_WAKE_REASON:-openclaw_smoke_test}"
PARTYCLIP_WAKE_COMMENT_ID="${PARTYCLIP_WAKE_COMMENT_ID:-}"
PARTYCLIP_APPROVAL_ID="${PARTYCLIP_APPROVAL_ID:-}"
PARTYCLIP_APPROVAL_STATUS="${PARTYCLIP_APPROVAL_STATUS:-}"
PARTYCLIP_LINKED_ISSUE_IDS="${PARTYCLIP_LINKED_ISSUE_IDS:-}"
...
PARTYCLIP_RUN_ID=${PARTYCLIP_RUN_ID}
PARTYCLIP_AGENT_ID=${PARTYCLIP_AGENT_ID}
PARTYCLIP_COMPANY_ID=${PARTYCLIP_COMPANY_ID}
PARTYCLIP_API_URL=${PARTYCLIP_API_URL}
PARTYCLIP_TASK_ID=${PARTYCLIP_TASK_ID}
PARTYCLIP_WAKE_REASON=${PARTYCLIP_WAKE_REASON}
PARTYCLIP_WAKE_COMMENT_ID=${PARTYCLIP_WAKE_COMMENT_ID}
PARTYCLIP_APPROVAL_ID=${PARTYCLIP_APPROVAL_ID}
PARTYCLIP_APPROVAL_STATUS=${PARTYCLIP_APPROVAL_STATUS}
PARTYCLIP_LINKED_ISSUE_IDS=${PARTYCLIP_LINKED_ISSUE_IDS}
...
PAYLOAD="$(jq -nc \
  ...
  --arg runId "$PARTYCLIP_RUN_ID" \
  --arg agentId "$PARTYCLIP_AGENT_ID" \
  --arg companyId "$PARTYCLIP_COMPANY_ID" \
  --arg apiUrl "$PARTYCLIP_API_URL" \
  --arg taskId "$PARTYCLIP_TASK_ID" \
  --arg wakeReason "$PARTYCLIP_WAKE_REASON" \
  --arg wakeCommentId "$PARTYCLIP_WAKE_COMMENT_ID" \
  --arg approvalId "$PARTYCLIP_APPROVAL_ID" \
  --arg approvalStatus "$PARTYCLIP_APPROVAL_STATUS" \
  --arg linkedIssueIds "$PARTYCLIP_LINKED_ISSUE_IDS" \
  '{
    model: $model,
    user: $user,
    input: $text,
    stream: true,
    metadata: {
      PARTYCLIP_RUN_ID: $runId,
      PARTYCLIP_AGENT_ID: $agentId,
      PARTYCLIP_COMPANY_ID: $companyId,
      PARTYCLIP_API_URL: $apiUrl,
      PARTYCLIP_TASK_ID: $taskId,
      PARTYCLIP_WAKE_REASON: $wakeReason,
      PARTYCLIP_WAKE_COMMENT_ID: $wakeCommentId,
