...
PARTYCLIP_API_URL="${PARTYCLIP_API_URL:-http://127.0.0.1:3100}"
...
PARTYCLIP_API_URL_FOR_OPENCLAW="${PARTYCLIP_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
if [[ -n "${PARTYCLIP_COOKIE:-}" ]]; then
  ...
  PARTYCLIP_BROWSER_ORIGIN="${PARTYCLIP_BROWSER_ORIGIN:-${PARTYCLIP_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${PARTYCLIP_BROWSER_ORIGIN}" -H "Referer: ${PARTYCLIP_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set PARTYCLIP_COOKIE or PARTYCLIP_AUTH_HEADER."
  fi
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/partyclip"
  local claimed_file="${workspace_dir}/partyclip-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/partyclip-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/partyclip-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/partyclip-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/partyclip"
    assert_status "200"
    {
      echo "PARTYCLIP_API_URL=${PARTYCLIP_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set PARTYCLIP_API_KEY=${AGENT_API_KEY} in your run context before running Paperclip heartbeat steps."
