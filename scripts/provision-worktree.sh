...
async function main() {
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.PARTYCLIP_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.PARTYCLIP_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.PARTYCLIP_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "PARTYCLIP_HOME=" + JSON.stringify(worktreeHome),
    "PARTYCLIP_INSTANCE_ID=" + JSON.stringify(instanceId),
    "PARTYCLIP_CONFIG=" + JSON.stringify(configPath),
    "PARTYCLIP_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "PARTYCLIP_IN_WORKTREE=true",
    "PARTYCLIP_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.PARTYCLIP_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("PARTYCLIP_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
