...
```sh
curl -sS -X POST "$PARTYCLIP_API_URL/api/companies/$PARTYCLIP_COMPANY_ID/skills/import" \
...
```sh
curl -sS -X POST "$PARTYCLIP_API_URL/api/companies/$PARTYCLIP_COMPANY_ID/skills/scan-projects" \
  -H "Authorization: Bearer $PARTYCLIP_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
```sh
curl -sS "$PARTYCLIP_API_URL/api/companies/$PARTYCLIP_COMPANY_ID/skills" \
  -H "Authorization: Bearer $PARTYCLIP_API_KEY"
```
...
```sh
curl -sS "$PARTYCLIP_API_URL/api/companies/$PARTYCLIP_COMPANY_ID/skills/<skill-id>" \
  -H "Authorization: Bearer $PARTYCLIP_API_KEY"

curl -sS "$PARTYCLIP_API_URL/api/companies/$PARTYCLIP_COMPANY_ID/skills/<skill-id>/files?path=SKILL.md" \
  -H "Authorization: Bearer $PARTYCLIP_API_KEY"
```
...
```sh
curl -sS -X POST "$PARTYCLIP_API_URL/api/agents/<agent-id>/skills/sync" \
  -H "Authorization: Bearer $PARTYCLIP_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "desiredSkills": [
...
```sh
curl -sS "$PARTYCLIP_API_URL/api/agents/<agent-id>/skills" \
  -H "Authorization: Bearer $PARTYCLIP_API_KEY"
```
...
```sh
curl -sS -X POST "$PARTYCLIP_API_URL/api/companies/$PARTYCLIP_COMPANY_ID/agents" \
  -H "Authorization: Bearer $PARTYCLIP_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "QA Browser Agent",
