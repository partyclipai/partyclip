...
|----------|-------------|
| `PARTYCLIP_AGENT_ID` | The agent's unique ID |
| `PARTYCLIP_COMPANY_ID` | The company the agent belongs to |
| `PARTYCLIP_API_URL` | Base URL for the Paperclip API |
| `PARTYCLIP_API_KEY` | Short-lived JWT for API authentication |
...
| `PARTYCLIP_WAKE_REASON` | Why the agent was woken (e.g. `issue_assigned`, `issue_comment_mentioned`) |
| `PARTYCLIP_WAKE_COMMENT_ID` | Specific comment that triggered this wake |
| `PARTYCLIP_APPROVAL_ID` | Approval that was resolved |
| `PARTYCLIP_APPROVAL_STATUS` | Approval decision (`approved`, `rejected`) |
