...
export PARTYCLIP_DOMAIN=partyclip.example.com   # your domain
...
aws ecr create-repository \
  --repository-name partyclip-server \
...
# Build
docker build -t partyclip-server .

# Tag and push
docker tag partyclip-server:latest \
...
aws rds create-db-subnet-group \
  --db-subnet-group-name partyclip-db-subnet \
  --db-subnet-group-description "Paperclip RDS subnets" \
  --subnet-ids $SUBNET_1 $SUBNET_2

aws rds create-db-instance \
  --db-instance-identifier partyclip-db \
  --db-instance-class db.t4g.micro \
  --engine postgres \
  --engine-version 17 \
  --master-username partyclip \
...
EFS_ID=$(aws efs create-file-system \
  ...
  --encrypted \
  --tags Key=Name,Value=partyclip-data \
...
aws secretsmanager create-secret \
  --name partyclip/database-url \
  --secret-string "$DATABASE_URL"

aws secretsmanager create-secret \
  --name partyclip/anthropic-api-key \
  --secret-string "YOUR_ANTHROPIC_KEY"

aws secretsmanager create-secret \
  --name partyclip/better-auth-secret \
  --secret-string "$AUTH_SECRET"

aws secretsmanager create-secret \
  --name partyclip/openai-api-key \
  --secret-string "YOUR_OPENAI_KEY"

aws secretsmanager create-secret \
  --name partyclip/github-token \
...
# Task execution role
aws iam create-role \
  --role-name partyclip-ecs-execution \
...
aws iam put-role-policy \
  ...
  --policy-document '{
    ...
    "Statement": [{
      ...
      "Resource": "arn:aws:secretsmanager:'$AWS_REGION':'$AWS_ACCOUNT_ID':secret:partyclip/*"
    }]
  }'

# Task role (application — add permissions as needed)
aws iam create-role \
  --role-name partyclip-ecs-task \
  --assume-role-policy-document '{
    "Version": "2012-10-17",
    "Statement": [{
      "Effect": "Allow",
...
```bash
aws ecs create-cluster --cluster-name partyclip
...
aws ecs create-service \
  --cluster partyclip \
  --service-name partyclip-server \
  --task-definition partyclip-server \
  --desired-count 1 \
  --launch-type FARGATE \
  ...
  --load-balancers '[{
    ...
    "containerName": "partyclip-server",
    "containerPort": 3100
  }]'
```
...
aws ecs describe-services \
  --cluster partyclip \
  --services partyclip-server \
  --query 'services[0].{desired:desiredCount,running:runningCount,status:status}'

# Check task health
aws ecs list-tasks --cluster partyclip --service-name partyclip-server
TASK_ARN=$(aws ecs list-tasks --cluster partyclip --service-name partyclip-server --query 'taskArns[0]' --output text)
aws ecs describe-tasks --cluster partyclip --tasks $TASK_ARN \
...
aws ecs list-task-definitions \
  --family-prefix partyclip-server \
  --sort DESC \
...
aws ecs update-service \
  --cluster partyclip \
  --service partyclip-server \
  --task-definition partyclip-server:<PREVIOUS_REVISION>
```
...
```bash
aws rds stop-db-instance --db-instance-identifier partyclip-db
aws rds start-db-instance --db-instance-identifier partyclip-db
```
...
aws ecs update-service --cluster partyclip --service partyclip-server --desired-count 0
aws ecs delete-service --cluster partyclip --service partyclip-server --force
aws ecs delete-cluster --cluster partyclip
...
aws rds delete-db-instance \
  --db-instance-identifier partyclip-db \
  --final-db-snapshot-identifier partyclip-db-final
aws rds wait db-instance-deleted --db-instance-identifier partyclip-db
aws rds delete-db-subnet-group --db-subnet-group-name partyclip-db-subnet
...
for s in database-url anthropic-api-key better-auth-secret openai-api-key github-token; do
  aws secretsmanager delete-secret --secret-id partyclip/$s --force-delete-without-recovery
done
...
# 7. ECR
aws ecr delete-repository --repository-name partyclip-server --force

# 8. IAM roles
aws iam delete-role-policy --role-name partyclip-ecs-execution --policy-name SecretsAccess
aws iam detach-role-policy --role-name partyclip-ecs-execution \
  --policy-arn arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy
aws iam delete-role --role-name partyclip-ecs-execution
aws iam delete-role --role-name partyclip-ecs-task
