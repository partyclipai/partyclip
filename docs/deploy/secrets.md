...
```
~/.partyclip/instances/default/secrets/master.key
```
...
|----------|-------------|
| `PARTYCLIP_SECRETS_MASTER_KEY` | 32-byte key as base64, hex, or raw string |
| `PARTYCLIP_SECRETS_MASTER_KEY_FILE` | Custom key file path |
| `PARTYCLIP_SECRETS_STRICT_MODE` | Set to `true` to enforce secret refs |
...
```sh
PARTYCLIP_SECRETS_STRICT_MODE=true
```
