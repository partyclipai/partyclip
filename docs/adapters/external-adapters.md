...
```json
{
  ...
  "dependencies": {
    "@partyclipai/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^22.0.0",
...
```ts
import type { ServerAdapterModule } from "@partyclipai/adapter-utils";
...
#### Available Helpers from `@partyclipai/adapter-utils`

| Helper | Purpose |
|--------|---------|
| `runChildProcess(command, opts)` | Spawn a child process with timeout, grace period, and streaming callbacks |
| `buildPaperclipEnv(agent)` | Inject `PARTYCLIP_*` environment variables |
...
```ts
import type { AdapterSessionCodec } from "@partyclipai/adapter-utils";
