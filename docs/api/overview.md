...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`PARTYCLIP_API_KEY`)
...
- Run audit trail: include `X-Partyclip-Run-Id` header on all mutating requests during heartbeats
