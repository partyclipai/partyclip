...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Partyclip-Signature` + `X-Partyclip-Timestamp` header pair matching the trigger's signing mode.
