/**
 * esbuild configuration for building the partyclipai CLI for npm.
 *
 * Bundles all workspace packages (@partyclipai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @partyclipai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@partyclipai/")) {
      externals.add(name);
    }
  }
