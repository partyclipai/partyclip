import { inferBindModeFromHost } from "@partyclipai/shared";
import type { PartyclipConfig } from "../config/schema.js";
import type { CheckResult } from "./index.js";

export function deploymentAuthCheck(config: PartyclipConfig): CheckResult {
  ...
  if (mode === "local_trusted") {
    if (bind !== "loopback") {
      return {
        ...
        canRepair: false,
        repairHint: "Run `partyclipai configure --section server` and choose Local trusted / loopback reachability",
      };
    }
    return {
      name: "Deployment/auth mode",
  ...
  const secret =
    ...
    process.env.PARTYCLIP_AGENT_JWT_SECRET?.trim();
  if (!secret) {
    return {
      name: "Deployment/auth mode",
      status: "fail",
  ...
  if (auth.baseUrlMode === "explicit" && !auth.publicBaseUrl) {
    return {
      ...
      canRepair: false,
      repairHint: "Run `partyclipai configure --section server` and provide a base URL",
    };
  }
  ...
  if (exposure === "public") {
    if (auth.baseUrlMode !== "explicit" || !auth.publicBaseUrl) {
      return {
        ...
        canRepair: false,
        repairHint: "Run `partyclipai configure --section server` and select public exposure",
      };
    }
    try {
      ...
      return {
        ...
        canRepair: false,
        repairHint: "Run `partyclipai configure --section server` and provide a valid URL",
      };
    }
  }
