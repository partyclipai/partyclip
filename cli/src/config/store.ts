...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".partyclip", DEFAULT_CONFIG_BASENAME);
...
export function resolveConfigPath(overridePath?: string): string {
  ...
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath(resolvePartyclipInstanceId());
}
...
export function readConfig(configPath?: string): PartyclipConfig | null {
  ...
  const parsed = partyclipConfigSchema.safeParse(migrated);
  if (!parsed.success) {
