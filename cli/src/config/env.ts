...
const JWT_SECRET_ENV_KEY = "PARTYCLIP_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# Partyclip environment variables",
    "# Generated by Partyclip CLI commands",
...
export function resolvePartyclipEnvFile(configPath?: string): string {
...
export function loadPartyclipEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergePartyclipEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readPartyclipEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writePartyclipEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergePartyclipEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readPartyclipEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writePartyclipEnvEntries(next, filePath);
  return next;
}
