...
const DATA_DIR_OPTION_HELP =
  "Partyclip data directory root (isolates state from ~/.partyclip)";

program
  .name("partyclipai")
  .description("Partyclip CLI — setup, diagnose, and configure your instance")
  .version(cliVersion);
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  loadPartyclipEnvFile(options.config);
...
program
  ...
  .option("--run", "Start Partyclip immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your Partyclip setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "partyclip")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Partyclip")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Partyclip server API")
