...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "partyclip-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.PARTYCLIP_HOME = path.join(root, "home");
    process.env.PARTYCLIP_INSTANCE_ID = "telemetry-test";
