...
function writeBaseConfig(configPath: string) {
  const base: PartyclipConfig = {
    ...
    database: {
      ...
      embeddedPostgresDataDir: "/tmp/partyclip-db",
      embeddedPostgresPort: 54329,
      backup: {
        enabled: true,
        intervalMinutes: 60,
        retentionDays: 30,
        dir: "/tmp/partyclip-backups",
      },
    },
    logging: {
      mode: "file",
      logDir: "/tmp/partyclip-logs",
    },
    server: {
    ...
    secrets: {
      ...
      strictMode: false,
      localEncrypted: { keyFilePath: "/tmp/partyclip-secrets/master.key" },
    },
  };
...
describe("allowed-hostname command", () => {
  it("adds and normalizes hostnames", async () => {
    ...
    const raw = JSON.parse(fs.readFileSync(configPath, "utf-8")) as PartyclipConfig;
