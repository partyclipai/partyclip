...
export async function runCommand(opts: RunOptions): Promise<void> {
  ...
  if (!configExists(configPath)) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      p.log.message(`Run ${pc.cyan("partyclipai onboard")} once, then retry ${pc.cyan("partyclipai run")}.`);
      process.exit(1);
    }
...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  if (process.env.PARTYCLIP_UI_DEV_MIDDLEWARE !== undefined) return;
  const normalized = entrypoint.replaceAll("\\", "/");
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@partyclipai/server/src/index.ts")) {
    process.env.PARTYCLIP_UI_DEV_MIDDLEWARE = "true";
  }
}
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  // Production mode: import the published @partyclipai/server package
  try {
    const mod = await import("@partyclipai/server");
    return await startServerFromModule(mod, "@partyclipai/server");
  } catch (err) {
...
function shouldGenerateBootstrapInviteAfterStart(config: PartyclipConfig): boolean {
