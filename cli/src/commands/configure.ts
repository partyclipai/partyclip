...
export async function configure(opts: {
  ...
  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `partyclipai onboard` first.");
    p.outro("");
    return;
  }

  let config: PartyclipConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because PARTYCLIP_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
