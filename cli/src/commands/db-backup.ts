...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@partyclipai/db";
import {
  expandHomePrefix,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" partyclip db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolvePartyclipInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "partyclip";
