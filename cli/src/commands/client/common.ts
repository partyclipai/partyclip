...
import { ApiRequestError, PartyclipApiClient } from "../../client/http.js";
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.PARTYCLIP_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.PARTYCLIP_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.PARTYCLIP_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set PARTYCLIP_COMPANY_ID, or set context profile companyId via `partyclipai context set`.",
    );
  }

  const api = new PartyclipApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.PARTYCLIP_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.PARTYCLIP_SERVER_PORT || "");
