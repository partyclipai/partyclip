...
import { resolvePartyclipInstanceId, resolvePartyclipInstanceRoot } from "../config/home.js";
...
export function resolveEnvLabSshStatePath(instanceId?: string): string {
  ...
  return path.resolve(
    resolvePartyclipInstanceRoot(resolvedInstanceId),
    "env-lab",
    "ssh-fixture",
    "state.json",
  );
...
export async function envLabDoctorCommand(opts: { instance?: string; json?: boolean }) {
  ...
  p.log.message(`Cleanup: ${pc.dim("pnpm partyclipai env-lab down")}`);
}
...
export function registerEnvLabCommands(program: Command) {
  ...
  envLab
    ...
    .option("-i, --instance <id>", "Partyclip instance id (default: current/default)")
