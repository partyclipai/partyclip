...
export const DEFAULT_WORKTREE_HOME = "~/.partyclip-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".partyclip");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: PartyclipConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "partyclip",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    PARTYCLIP_HOME: paths.homeDir,
    PARTYCLIP_INSTANCE_ID: paths.instanceId,
    PARTYCLIP_CONFIG: paths.configPath,
    PARTYCLIP_CONTEXT: paths.contextPath,
    PARTYCLIP_IN_WORKTREE: "true",
    ...(branding?.name ? { PARTYCLIP_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { PARTYCLIP_WORKTREE_COLOR: branding.color } : {}),
  };
}
