...
type OnboardDefaults = Pick<PartyclipConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const TAILNET_BIND_WARNING =
  "No Tailscale address was detected during setup. The saved config will stay on loopback until Tailscale is available or PARTYCLIP_TAILNET_BIND_HOST is set.";

const ONBOARD_ENV_KEYS = [
  "PARTYCLIP_PUBLIC_URL",
  "DATABASE_URL",
  "PARTYCLIP_DB_BACKUP_ENABLED",
  "PARTYCLIP_DB_BACKUP_INTERVAL_MINUTES",
  "PARTYCLIP_DB_BACKUP_RETENTION_DAYS",
  "PARTYCLIP_DB_BACKUP_DIR",
  "PARTYCLIP_DEPLOYMENT_MODE",
  "PARTYCLIP_DEPLOYMENT_EXPOSURE",
  "PARTYCLIP_BIND",
  "PARTYCLIP_BIND_HOST",
  "PARTYCLIP_TAILNET_BIND_HOST",
  "HOST",
  "PORT",
  "SERVE_UI",
  "PARTYCLIP_ALLOWED_HOSTNAMES",
  "PARTYCLIP_AUTH_BASE_URL_MODE",
  "PARTYCLIP_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "PARTYCLIP_STORAGE_PROVIDER",
  "PARTYCLIP_STORAGE_LOCAL_DIR",
  "PARTYCLIP_STORAGE_S3_BUCKET",
  "PARTYCLIP_STORAGE_S3_REGION",
  "PARTYCLIP_STORAGE_S3_ENDPOINT",
  "PARTYCLIP_STORAGE_S3_PREFIX",
  "PARTYCLIP_STORAGE_S3_FORCE_PATH_STYLE",
  "PARTYCLIP_SECRETS_PROVIDER",
  "PARTYCLIP_SECRETS_STRICT_MODE",
  "PARTYCLIP_SECRETS_MASTER_KEY_FILE",
] as const;
...
function describeServerBinding(server: Pick<PartyclipConfig["server"], "bind" | "customBindHost" | "host" | "port">): string {
...
function quickstartDefaultsFromEnv(opts?: { preferTrustedLocal?: boolean }): {
  ...
  const publicUrl = preferTrustedLocal
    ? undefined
    : (
      process.env.PARTYCLIP_PUBLIC_URL?.trim() ||
      process.env.PARTYCLIP_AUTH_PUBLIC_BASE_URL?.trim() ||
  ...
  const deploymentMode = preferTrustedLocal
    ? "local_trusted"
    : (parseEnumFromEnv<DeploymentMode>(process.env.PARTYCLIP_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted");
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.PARTYCLIP_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  const deploymentExposure =
    deploymentMode === "local_trusted" ? "private" : (deploymentExposureFromEnv ?? "private");
  const bindFromEnv = parseEnumFromEnv<BindMode>(process.env.PARTYCLIP_BIND, BIND_MODES);
  const customBindHostFromEnv = process.env.PARTYCLIP_BIND_HOST?.trim() || undefined;
  ...
  const resolvedBind = resolveRuntimeBind({
    ...
    tailnetBindHost: process.env.PARTYCLIP_TAILNET_BIND_HOST?.trim(),
  });
  const authPublicBaseUrl = publicUrl;
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.PARTYCLIP_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.PARTYCLIP_ALLOWED_HOSTNAMES
    ? process.env.PARTYCLIP_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.PARTYCLIP_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.PARTYCLIP_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.PARTYCLIP_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.PARTYCLIP_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.PARTYCLIP_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.PARTYCLIP_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.PARTYCLIP_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.PARTYCLIP_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.PARTYCLIP_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.PARTYCLIP_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.PARTYCLIP_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.PARTYCLIP_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.PARTYCLIP_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.PARTYCLIP_SECRETS_MASTER_KEY_FILE) ??
  ...
  if (deploymentMode === "local_trusted" && process.env.PARTYCLIP_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "PARTYCLIP_DEPLOYMENT_EXPOSURE",
      reason: "Ignored because deployment mode local_trusted always forces private exposure",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.PARTYCLIP_BIND !== undefined) {
    ignoredEnvKeys.push({
      key: "PARTYCLIP_BIND",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.PARTYCLIP_BIND_HOST !== undefined) {
    ignoredEnvKeys.push({
      key: "PARTYCLIP_BIND_HOST",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
...
function canCreateBootstrapInviteImmediately(config: Pick<PartyclipConfig, "database" | "server">): boolean {
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  let existingConfig: PartyclipConfig | null = null;
  ...
  if (existingConfig) {
    ...
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("PARTYCLIP_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.PARTYCLIP_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("PARTYCLIP_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("PARTYCLIP_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: PARTYCLIP_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("partyclipai run")}`,
        `Reconfigure later: ${pc.cyan("partyclipai configure")}`,
        `Diagnose setup: ${pc.cyan("partyclipai doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.PARTYCLIP_OPEN_ON_LISTEN = "true";
  ...
  let llm: PartyclipConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@partyclipai/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `partyclipai doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("partyclipai run")}`,
        `Then: ${pc.cyan("partyclipai auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
