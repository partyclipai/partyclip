...
export function readConfigFromEnv(env: NodeJS.ProcessEnv = process.env): PartyclipMcpConfig {
  ...
  const apiKey = nonEmpty(env.PARTYCLIP_API_KEY);
  if (!apiKey) {
    throw new Error("Missing PARTYCLIP_API_KEY");
  }

  return {
    apiUrl: normalizeApiUrl(apiUrl),
    apiKey,
    companyId: nonEmpty(env.PARTYCLIP_COMPANY_ID),
    agentId: nonEmpty(env.PARTYCLIP_AGENT_ID),
    runId: nonEmpty(env.PARTYCLIP_RUN_ID),
  };
}
