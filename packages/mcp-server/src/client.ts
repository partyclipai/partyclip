import type { PartyclipMcpConfig } from "./config.js";

export class PartyclipApiError extends Error {
...
export class PartyclipApiClient {
  constructor(private readonly config: PartyclipMcpConfig) {}
  ...
  resolveCompanyId(companyId?: string | null): string {
    ...
    if (!resolved) {
      throw new Error("companyId is required because PARTYCLIP_COMPANY_ID is not set");
    }
    return resolved;
  }
  ...
  resolveAgentId(agentId?: string | null): string {
    ...
    if (!resolved) {
      throw new Error("agentId is required because PARTYCLIP_AGENT_ID is not set");
    }
    return resolved;
  }
  ...
  async requestJson<T>(method: string, path: string, options: JsonRequestOptions = {}): Promise<T> {
    ...
    if ((options.includeRunId ?? isWriteMethod(method)) && this.config.runId) {
      headers["X-Partyclip-Run-Id"] = this.config.runId;
    }
