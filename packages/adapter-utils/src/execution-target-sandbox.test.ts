...
describe("sandbox adapter execution targets", () => {
  ...
  it("starts a localhost Paperclip bridge for sandbox targets in bridge mode", async () => {
    ...
    try {
      ...
      expect(bridge?.env.PARTYCLIP_API_URL).toMatch(/^http:\/\/127\.0\.0\.1:\d+$/);
      expect(bridge?.env.PARTYCLIP_API_KEY).not.toBe("real-run-jwt");
      expect(bridge?.env.PARTYCLIP_API_BRIDGE_MODE).toBe("queue_v1");

      const response = await fetch(`${bridge!.env.PARTYCLIP_API_URL}/api/agents/me`, {
        headers: {
          authorization: `Bearer ${bridge!.env.PARTYCLIP_API_KEY}`,
