...
export function buildSandboxCallbackBridgeEnv(input: {
  ...
  return {
    PARTYCLIP_API_BRIDGE_MODE: "queue_v1",
    PARTYCLIP_BRIDGE_QUEUE_DIR: input.queueDir,
    PARTYCLIP_BRIDGE_TOKEN: input.bridgeToken,
    PARTYCLIP_BRIDGE_HOST: input.host?.trim() || "127.0.0.1",
    PARTYCLIP_BRIDGE_PORT: String(input.port && input.port > 0 ? Math.trunc(input.port) : 0),
    PARTYCLIP_BRIDGE_POLL_INTERVAL_MS: String(
      normalizeTimeoutMs(input.pollIntervalMs, DEFAULT_BRIDGE_POLL_INTERVAL_MS),
    ),
    PARTYCLIP_BRIDGE_RESPONSE_TIMEOUT_MS: String(
      normalizeTimeoutMs(input.responseTimeoutMs, DEFAULT_BRIDGE_RESPONSE_TIMEOUT_MS),
    ),
    PARTYCLIP_BRIDGE_MAX_QUEUE_DEPTH: String(
      normalizeTimeoutMs(input.maxQueueDepth, DEFAULT_BRIDGE_MAX_QUEUE_DEPTH),
    ),
    PARTYCLIP_BRIDGE_MAX_BODY_BYTES: String(
...
const queueDir = process.env.PARTYCLIP_BRIDGE_QUEUE_DIR;
const bridgeToken = process.env.PARTYCLIP_BRIDGE_TOKEN;
const host = process.env.PARTYCLIP_BRIDGE_HOST || "127.0.0.1";
const port = Number(process.env.PARTYCLIP_BRIDGE_PORT || "0");
const pollIntervalMs = Number(process.env.PARTYCLIP_BRIDGE_POLL_INTERVAL_MS || "100");
const responseTimeoutMs = Number(process.env.PARTYCLIP_BRIDGE_RESPONSE_TIMEOUT_MS || "30000");
const maxQueueDepth = Number(process.env.PARTYCLIP_BRIDGE_MAX_QUEUE_DEPTH || "${DEFAULT_BRIDGE_MAX_QUEUE_DEPTH}");
const maxBodyBytes = Number(process.env.PARTYCLIP_BRIDGE_MAX_BODY_BYTES || "${DEFAULT_BRIDGE_MAX_BODY_BYTES}");
const allowedHeaders = new Set(${JSON.stringify([...DEFAULT_SANDBOX_CALLBACK_BRIDGE_HEADER_ALLOWLIST])});

if (!queueDir || !bridgeToken) {
  throw new Error("PARTYCLIP_BRIDGE_QUEUE_DIR and PARTYCLIP_BRIDGE_TOKEN are required.");
}
