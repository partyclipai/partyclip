...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "partyclip-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function resolvePaperclipInstanceId(): string {
  const raw = process.env.PARTYCLIP_INSTANCE_ID?.trim() || DEFAULT_INSTANCE_ID;
  if (!INSTANCE_ID_RE.test(raw)) {
    throw new Error(`Invalid PARTYCLIP_INSTANCE_ID '${raw}'.`);
  }
  return raw;
}
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".partyclip", CONFIG_BASENAME);
...
function resolvePaperclipConfigPath(): string {
  if (process.env.PARTYCLIP_CONFIG?.trim()) {
    return path.resolve(process.env.PARTYCLIP_CONFIG.trim());
  }
