...
async function createTempDatabase(): Promise<string> {
  const db = await startEmbeddedPostgresTestDatabase("partyclip-db-backup-");
...
describe("createBufferedTextFileWriter", () => {
  it("preserves line boundaries across buffered flushes", async () => {
    const tempDir = createTempDir("partyclip-buffered-writer-");
...
describeEmbeddedPostgres("runDatabaseBackup", () => {
  it(
    ...
    async () => {
      ...
      const restoreConnectionString = await createSiblingDatabase(
        ...
        "partyclip_restore_target",
      );
      const backupDir = createTempDir("partyclip-db-backup-output-");
  ...
  it(
    ...
    async () => {
      ...
      const restoreConnectionString = await createSiblingDatabase(
        sourceConnectionString,
        "partyclip_full_logical_restore_target",
      );
      const backupDir = createTempDir("partyclip-db-full-logical-backup-");
      const sourceSql = postgres(sourceConnectionString, { max: 1, onnotice: () => {} });
      const restoreSql = postgres(restoreConnectionString, { max: 1, onnotice: () => {} });
      ...
      try {
        await sourceSql.unsafe(`
          ...
          VALUES ('partyclip-migration-history', 1770000000000);
        `);
        await sourceSql.unsafe(`
        ...
        const result = await runDatabaseBackup({
          ...
          backupDir,
          retention: { dailyDays: 7, weeklyWeeks: 4, monthlyMonths: 1 },
          filenamePrefix: "partyclip-full-logical-test",
          backupEngine: "javascript",
        ...
        const migrationRows = await restoreSql.unsafe<{ hash: string }[]>(`
          ...
          WHERE "hash" = 'partyclip-migration-history'
        `);
        expect(migrationRows).toEqual([{ hash: "partyclip-migration-history" }]);
  ...
  it(
    ...
    async () => {
      ...
      const restoreSql = postgres(restoreConnectionString, { max: 1, onnotice: () => {} });
      const backupDir = createTempDir("partyclip-db-restore-manual-");
      ...
      try {
        await fs.promises.writeFile(
          ...
          [
            ...
            "",
            "BEGIN;",
            "-- partyclip statement breakpoint 69f6f3f1-42fd-46a6-bf17-d1d85f8f3900",
