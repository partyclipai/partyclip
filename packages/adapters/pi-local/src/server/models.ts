...
import type { AdapterModel } from "@partyclipai/adapter-utils";
import { asString, runChildProcess } from "@partyclipai/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.PARTYCLIP_PI_COMMAND === "string" &&
    process.env.PARTYCLIP_PI_COMMAND.trim().length > 0
      ? process.env.PARTYCLIP_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["PARTYCLIP_", "npm_", "NPM_"] as const;
