...
function renderPaperclipEnvNote(env: Record<string, string>): string {
  ...
  return [
    ...
    `The following PARTYCLIP_* environment variables are available in this run: ${paperclipKeys.join(", ")}`,
...
function renderApiAccessNote(env: Record<string, string>): string {
  ...
  return [
    ...
    "POST/PATCH example:",
    `  run_shell_command({ command: "curl -s -X POST -H \\"Authorization: Bearer $PARTYCLIP_API_KEY\\" -H 'Content-Type: application/json' -H \\"X-Paperclip-Run-Id: $PARTYCLIP_RUN_ID\\" -d '{...}' \\"$PARTYCLIP_API_URL/api/issues/{id}/checkout\\"" })`,
    "",
    "",
  ].join("\n");
}
