import type { AdapterModelProfileDefinition } from "@partyclipai/adapter-utils";
...
- Paperclip injects desired local skills into the effective CODEX_HOME/skills/ directory at execution time so Codex can discover "$partyclip" and related skills without polluting the project working directory. In managed-home mode (the default) this is ~/.partyclip/instances/<id>/companies/<companyId>/codex-home/skills/; when CODEX_HOME is explicitly overridden in adapter config, that override is used instead.
...
- When Paperclip realizes a workspace/runtime for a run, it injects PARTYCLIP_WORKSPACE_* and PARTYCLIP_RUNTIME_* env vars for agent-side tooling.
`;
