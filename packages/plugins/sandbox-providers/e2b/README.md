...
These commands assume the repo root has already been installed once so the local `@partyclipai/plugin-sdk` workspace package is available to the compiler during development.

## Package layout

- `src/manifest.ts` declares the sandbox-provider driver metadata
- `src/plugin.ts` implements the environment lifecycle hooks
- `partyclipPlugin.manifest` and `partyclipPlugin.worker` point the host at the built plugin entrypoints in `dist/`
