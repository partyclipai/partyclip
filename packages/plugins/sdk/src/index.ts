/**
 * `@partyclipai/plugin-sdk` — Paperclip plugin worker-side SDK.
 *
 * This is the main entrypoint for plugin worker code.  For plugin UI bundles,
 * import from `@partyclipai/plugin-sdk/ui` instead.
 *
 * @example
 * ```ts
 * // Plugin worker entrypoint (dist/worker.ts)
 * import { definePlugin, runWorker, z } from "@partyclipai/plugin-sdk";
 *
...
// Manifest and constant types re-exported from @partyclipai/shared
// Plugin authors import manifest types from here so they have a single
// dependency (@partyclipai/plugin-sdk) for all plugin authoring needs.
export type {
 ...
 * @example
 * ```ts
 * import { z } from "@partyclipai/plugin-sdk";
 *
