# @partyclipai/plugin-hello-world-example
...
```bash
pnpm --filter @partyclipai/plugin-hello-world-example build
pnpm partyclipai plugin install ./packages/plugins/examples/plugin-hello-world-example
```
...
- **Reinstall after pulling.** If you installed a plugin by local path before the server stored `package_path`, the plugin may show status **error** (worker not found). Uninstall and install again so the server persists the path and can activate the plugin:  
  `pnpm partyclipai plugin uninstall partyclip.hello-world-example --force` then  
  `pnpm partyclipai plugin install ./packages/plugins/examples/plugin-hello-world-example`.
