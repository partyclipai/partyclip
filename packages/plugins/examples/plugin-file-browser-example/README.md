...
```bash
pnpm --filter @partyclipai/plugin-file-browser-example build
pnpm partyclipai plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm partyclipai plugin uninstall partyclip-file-browser-example --force
```
...
- Optional: use `partyclip-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
