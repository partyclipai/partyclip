...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Partyclip API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "partyclipApiUrl",
                  String(config.partyclipApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "partyclipApiUrl", v || undefined)}
              immediate
              className={inputClass}
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "partyclip"))}
                onCommit={(v) => mark("adapterConfig", "sessionKey", v || undefined)}
                immediate
                className={inputClass}
                placeholder="partyclip"
              />
            </Field>
          )}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Partyclip persists a device key during onboarding so pairing approvals
