...
import type { Company } from "@partyclipai/shared";
...
describe("CompanyProvider", () => {
  ...
  it("does not expose a stale stored company id before companies load", async () => {
    localStorage.setItem("partyclip.selectedCompanyId", "stale-company");
  ...
  it("replaces a stale stored company id with the first loaded company", async () => {
    ...
    expect(localStorage.getItem("partyclip.selectedCompanyId")).toBe("company-1");
  });
});
