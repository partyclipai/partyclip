...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = PARTYCLIP_SPRITES[(baseCol + baseRow) % PARTYCLIP_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = PARTYCLIP_SPRITES[Math.floor(Math.random() * PARTYCLIP_SPRITES.length)]!;
