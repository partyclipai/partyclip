...
packages/adapters/<name>/
  src/
    ...
    cli/
      ...
      format-event.ts   # Colored terminal output for `partyclipai run --watch`
  package.json
  tsconfig.json
```
...
## 2. Shared Types (`@partyclipai/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@partyclipai/adapter-utils` (types) or `@partyclipai/adapter-utils/server-utils` (runtime helpers).
...
```json
{
  ...
  "dependencies": {
    "@partyclipai/adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@partyclipai/adapter-utils/server-utils`)
2. **Build environment** — call `buildPaperclipEnv(agent)` then layer in `PARTYCLIP_RUN_ID`, context vars (`PARTYCLIP_TASK_ID`, `PARTYCLIP_WAKE_REASON`, `PARTYCLIP_WAKE_COMMENT_ID`, `PARTYCLIP_APPROVAL_ID`, `PARTYCLIP_APPROVAL_STATUS`, `PARTYCLIP_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
| `PARTYCLIP_API_URL` | Server's own URL |
| `PARTYCLIP_RUN_ID` | Current run id |
| `PARTYCLIP_TASK_ID` | `context.taskId` or `context.issueId` |
| `PARTYCLIP_WAKE_REASON` | `context.wakeReason` |
| `PARTYCLIP_WAKE_COMMENT_ID` | `context.wakeCommentId` or `context.commentId` |
| `PARTYCLIP_APPROVAL_ID` | `context.approvalId` |
| `PARTYCLIP_APPROVAL_STATUS` | `context.approvalStatus` |
| `PARTYCLIP_LINKED_ISSUE_IDS` | `context.issueIds` (comma-separated) |
| `PARTYCLIP_API_KEY` | `authToken` (if no explicit key in config) |
...
Pretty-prints stdout lines for `partyclipai run --watch`. Use `picocolors` for coloring.
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@partyclipai/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@partyclipai/adapter-my-agent";
...
import { parseMyAgentStdoutLine } from "@partyclipai/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@partyclipai/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@partyclipai/adapter-my-agent/cli";
...
Import from `@partyclipai/adapter-utils/server-utils`:
...
| `buildPaperclipEnv(agent)` | Standard `PARTYCLIP_*` env vars |
...
- Package name: `@partyclipai/adapter-<kebab-name>`
...
- Default prompt should use `DEFAULT_PARTYCLIP_AGENT_PROMPT_TEMPLATE` from `@partyclipai/adapter-utils/server-utils` so local adapters share Paperclip's execution contract: act in the same heartbeat, avoid planning-only exits unless requested, leave durable progress and a next action, use child issues instead of polling, mark blockers with owner/action, and respect governance boundaries.
...
Paperclip ships shared skills (in the repo's top-level `skills/` directory) that agents need at runtime — things like the `partyclip` API skill and the `partyclip-create-agent` workflow skill. Each adapter is responsible for making these skills discoverable by its agent runtime **without polluting the agent's working directory**.
...
1. At execution time, create a fresh tmpdir: `mkdtemp("partyclip-skills-")`
...
async function buildSkillsDir(): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "partyclip-skills-"));
  const target = path.join(tmp, ".claude", "skills");
  await fs.mkdir(target, { recursive: true });
  const entries = await fs.readdir(PARTYCLIP_SKILLS_DIR, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isDirectory()) {
      await fs.symlink(
        path.join(PARTYCLIP_SKILLS_DIR, entry.name),
...
- `PARTYCLIP_API_KEY` is injected by the server into the process environment, not the prompt
